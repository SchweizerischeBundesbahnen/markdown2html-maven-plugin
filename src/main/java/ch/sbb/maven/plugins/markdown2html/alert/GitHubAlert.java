...
/**
 * The five kinds of alert GitHub renders a blockquote as when its first line marks one, together with the
 * markup it renders them in - the class names {@code github-markdown-css} styles and the octicon it puts in
 * front of the title.
 */
@Getter
...
public enum GitHubAlert {
    ...
    private static final String ICON = "<svg data-component=\"Octicon\" class=\"octicon octicon-%s mr-2\""
    ...
    public static @Nullable GitHubAlert ofMarker(@NotNull String marker) {
        ...
        String name = marker.substring(2, marker.length() - 1);
        for (GitHubAlert alert : values()) {
